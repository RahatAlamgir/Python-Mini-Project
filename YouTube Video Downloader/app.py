...
class YouTubeDownloaderApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.url_label = ctk.CTkLabel(self.url_frame, text="Video URL", font=ctk.CTkFont(size=12, weight="bold"))
        self.url_label.pack(anchor="w", padx=15, pady=(8, 2))

        self.url_entry = ctk.CTkEntry(
            self.url_frame, placeholder_text="Paste YouTube URL here...", height=35
        )
        self.url_entry.pack(fill="x", padx=15, pady=(0, 10))
        self.url_entry.bind("<KeyRelease>", self.on_url_change)

        # Thumbnail & Title Preview Frame
        ...
        self.thumbnail_label = ctk.CTkLabel(
            self.preview_frame, text="Paste a valid YouTube URL to preview", text_color="gray"
        )
        self.thumbnail_label.pack(pady=(10, 5))

        self.video_title_label = ctk.CTkLabel(
            self.preview_frame, text="", font=ctk.CTkFont(size=13, weight="bold"), wraplength=520
        )
        ...
        self.rename_label = ctk.CTkLabel(self.options_frame, text="Custom Name:", font=ctk.CTkFont(size=12))
        self.rename_label.grid(row=0, column=0, padx=15, pady=(10, 4), sticky="w")

        self.rename_entry = ctk.CTkEntry(
            self.options_frame, placeholder_text="Leave blank to use video title", height=32
        )
        self.rename_entry.grid(row=0, column=1, columnspan=3, padx=15, pady=(10, 4), sticky="ew")

        # Type (Video / Audio Only)
        self.type_label = ctk.CTkLabel(self.options_frame, text="Mode:", font=ctk.CTkFont(size=12))
        self.type_label.grid(row=1, column=0, padx=15, pady=4, sticky="w")
        ...
        self.type_dropdown.grid(row=1, column=1, padx=(15, 5), pady=4, sticky="w")

        # Compact Quality Dropdown
        self.quality_label = ctk.CTkLabel(self.options_frame, text="Quality:", font=ctk.CTkFont(size=12))
        self.quality_label.grid(row=1, column=2, padx=(10, 5), pady=4, sticky="w")
        ...
        self.quality_dropdown.grid(row=1, column=3, padx=(0, 15), pady=4, sticky="w")

        # Format Selection
        self.format_label = ctk.CTkLabel(self.options_frame, text="Format:", font=ctk.CTkFont(size=12))
        self.format_label.grid(row=2, column=0, padx=15, pady=4, sticky="w")
        ...
        self.format_dropdown.grid(row=2, column=1, padx=(15, 5), pady=4, sticky="w")

        # Subtitle Selection
        self.subtitle_label = ctk.CTkLabel(self.options_frame, text="Subtitles:", font=ctk.CTkFont(size=12))
        self.subtitle_label.grid(row=2, column=2, padx=(10, 5), pady=4, sticky="w")
        ...
        self.subtitle_dropdown.grid(row=2, column=3, padx=(0, 15), pady=4, sticky="w")
        ...
        self.path_button.grid(row=3, column=0, padx=15, pady=(4, 10), sticky="w")

        self.path_label = ctk.CTkLabel(
            self.options_frame, text=self.download_path, text_color="gray", anchor="w"
        )
        self.path_label.grid(row=3, column=1, columnspan=3, padx=15, pady=(4, 10), sticky="ew")
        ...
        # Trigger initial state UI for mode selection after controls exist
    ...
    def on_type_change(self, choice):
        if choice == "Audio Only":
            ...
            self.download_btn.configure(text="Download MP3 / Audio")
        else:
    ...
    def fetch_preview(self, url):
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                self.original_video_title = info.get('title', 'Unknown Title')
            ...
            if thumb_url:
                ...
                ctk_img = ctk.CTkImage(light_image=pil_img, dark_image=pil_img, size=(180, 101))
    ...
    def download_video(self, url):
        ...
        title_template = custom_name if custom_name else "%(title)s"
        ...
        if mode == "Audio Only":
            output_file_pattern = f"{title_template}.%(ext)s"
            ...
            output_file_pattern = f"{title_template} - %(height)sp.%(ext)s"
            output_template = os.path.join(self.download_path, output_file_pattern)
            ...
            if "Best" in quality:
                ...
                height = match.group() if match else "1080"
                format_str = (
